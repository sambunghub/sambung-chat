<script lang="ts">
	...
	async function testImportTheme(exportData: ThemeExport, testId: string) {
		...
		try {
			...
			if (result.theme.name !== exportData.name) {
				throw new Error(
					`Name mismatch: expected "${exportData.name}", got "${result.theme.name}"`
				);
			}
	...
	async function runAllTests() {
		...
		addLog(
			`Success Rate: ${((testSummary.passed / testSummary.total) * 100).toFixed(1)}%`
		);
	...
	<div class="flex items-center justify-between border-b pb-4">
		<div>
			<h1 class="text-3xl font-bold flex items-center gap-2">
				<TestTube class="w-8 h-8 text-primary" />
		...
		<div class="flex items-center gap-4">
			...
			<Button onclick={runAllTests} disabled={isRunning} size="lg">
				<RefreshCw class="w-4 h-4 mr-2 {isRunning ? 'animate-spin' : ''}" />
			...
			<Card>
				...
				<CardContent>
					<div class="space-y-2">
						{#each tests as test}
							<div
								class="flex items-center justify-between p-3 rounded-lg border {test.status === 'passed'
							...
							>
								<div class="flex items-center gap-3 flex-1">
									...
									{:else}
										<div class="w-5 h-5 rounded-full border-2 border-muted-foreground/30" />
									{/if}
									<div class="flex-1">
								...
								{#if test.duration}
									<div class="text-xs text-muted-foreground font-mono">{test.duration}ms</div>
								{/if}
							</div>
						{/each}
					</div>
		...
		<div class="space-y-4">
				...
				<CardContent class="space-y-3 text-sm">
					<div>
						...
						<div class="text-muted-foreground">
							6 themes (3 built-in + 3 custom)
						</div>
					</div>
					<div>
						<div class="font-semibold mb-1">Import Tests</div>
						<div class="text-muted-foreground">4 theme imports</div>
					</div>
					<div>
						<div class="font-semibold mb-1">Round-trip Tests</div>
						<div class="text-muted-foreground">3 data integrity tests</div>
					</div>
					<div>
						<div class="font-semibold mb-1">Error Handling</div>
						<div class="text-muted-foreground">3 validation tests</div>
					</div>
					<div>
						<div class="font-semibold mb-1">Format Tests</div>
			...
			<Card>
				...
				<CardContent class="space-y-2 text-sm">
					...
					<div class="font-semibold pt-2">Custom Themes:</div>
					<ul class="list-disc list-inside text-muted-foreground space-y-1 ml-2">
						<li>Purple Dream</li>
						<li>Ocean Blue</li>
						<li>Forest Green</li>
			...
			<Card>
				...
				<CardContent>
					<div class="space-y-1 max-h-96 overflow-y-auto text-xs font-mono">
						...
						{/each}
						{#if testLog.length === 0}
							<div class="text-muted-foreground text-center py-4">No activity yet</div>
						{/if}
					</div>
				</CardContent>
			</Card>
