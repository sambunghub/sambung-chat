  ...
  <div class="space-y-3">
    ...
    {#if showValues}
      <div class="border-border bg-muted/30 rounded-md border p-2">
        <code class="font-mono text-xs">{formatHSL(hue, saturation, lightness)}</code>
      </div>
    {/if}
    ...
    <div
      ...
      style="background-color: {cssColor}; border-color: hsl({hue}, {saturation}%, {Math.max(
        0,
        lightness - 20
      )}%);"
    >
      <span class="text-sm font-medium" style="color: {textColor};"> Sample Text </span>
    </div>
  </div>
</div>
