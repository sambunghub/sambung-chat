<script lang="ts">
  ...
  let { class: className, showPreview = true }: Props = $props();

  // Local state for immediate feedback before store update
  let localValue = $state<number>(appearance.currentSettings.sidebarWidth || 280);
  let isDragging = $state(false);

  // Sync local state with store
  $effect(() => {
    if (!isDragging) {
      localValue = appearance.currentSettings.sidebarWidth || 280;
    }
  });
  ...
  function handleChange(event: Event) {
    ...
    // Update the store
    appearance.updateSetting('sidebarWidth', value);
  }
  ...
  function decreaseWidth() {
    ...
    localValue = newValue;
    appearance.updateSetting('sidebarWidth', newValue);
  }
  ...
  function setPresetWidth(width: number) {
    localValue = width;
    appearance.updateSetting('sidebarWidth', width);
  }

  /**
   * Calculate percentage for slider position
   */
  const sliderPercentage = $derived(((localValue - MIN_WIDTH) / (MAX_WIDTH - MIN_WIDTH)) * 100);
  ...
  <CardContent class="space-y-6">
    ...
    {#if showPreview}
      ...
      <div class="border-border bg-muted/30 rounded-lg border p-4">
        <p class="text-muted-foreground mb-3 text-xs font-medium uppercase">Preview</p>
        <div class="border-border relative mx-auto h-24 w-full rounded-md border-2 border-dashed">
          <!-- Sidebar preview -->
          <div
            class="bg-primary/20 border-border absolute top-0 left-0 flex h-full items-center justify-center rounded-l-md border-r transition-all duration-200"
          ...
          <div
            class="absolute top-0 right-0 flex h-full items-center justify-center rounded-r-md"
