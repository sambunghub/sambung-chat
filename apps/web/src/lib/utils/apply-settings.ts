 ...
 */
export function applySidebarWidth(sidebarWidth: number): void {
  if (!browser) return;
...
async function appearanceStoreInitialized(): Promise<void> {
  ...
  // Get current loading state by checking the store
  let isLoading = false;
  ...
  while (isLoading && elapsed < maxWaitTime) {
    await new Promise((resolve) => setTimeout(resolve, pollInterval));
    elapsed += pollInterval;

    // Re-check loading state
    const unsubscribe2 = appearance.subscribe((state) => {
      isLoading = state.loading;
    });
    unsubscribe2();
  }
...
export function getAppearanceVariables(): Record<string, string> {
  ...
  const variables: Record<string, string> = {
    [APPEARANCE_VARIABLES.fontSize]: computedStyle
      .getPropertyValue(APPEARANCE_VARIABLES.fontSize)
      .trim(),
    [APPEARANCE_VARIABLES.fontFamily]: computedStyle
      .getPropertyValue(APPEARANCE_VARIABLES.fontFamily)
      .trim(),
    [APPEARANCE_VARIABLES.sidebarWidth]: computedStyle
      .getPropertyValue(APPEARANCE_VARIABLES.sidebarWidth)
      .trim(),
  };
