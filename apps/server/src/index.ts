  ...
  try {
    ...
    if (rpcResult.matched) {
      ...
      if (
        ...
        // Note: Cast status and headers to satisfy Hono's type constraints
        const statusCode = rpcResult.response.status as
          | 100
          | 101
          | 102
          | 103
          ...
          | 510
          | 511;
        const responseHeaders = rpcResult.response.headers as unknown as Record<
          string,
          string | string[]
        >;

        return c.newResponse(JSON.stringify(cleanBody), statusCode, responseHeaders);
      }
