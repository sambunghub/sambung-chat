...
test.describe('Search Functionality', () => {
	...
	test.describe('Provider Filter', () => {
		...
		test('should open provider dropdown when clicked', async ({ page }) => {
			...
			const dropdown = page.locator('[role="menu"]').filter({ hasText: /OpenAI|Anthropic|Google/i });
		...
		test('should have "Clear providers" option when providers are selected', async ({ page }) => {
			...
			if (hasOptions) {
				...
				const clearOption = page.locator('[role="menuitem"]').filter({ hasText: /Clear providers/ });
	...
	test.describe('Model Filter', () => {
		test('should display model filter dropdown when models are configured', async ({ page }) => {
			const modelFilter = page.locator('button').filter({ hasText: /All Models|model.*selected/ }).first();
	...
	test.describe('Clear All Filters', () => {
		...
		test('should not display "Clear All Filters" button when no filters are active', async ({ page }) => {
