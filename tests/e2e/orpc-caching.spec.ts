...
test.describe.skip('ORPC Caching - Folder Endpoints', () => {
  ...
  test('should return 304 Not Modified for matching ETag on folder.getAll', async ({ request }) => {
...
test.describe.skip('ORPC Caching - Model Endpoints', () => {
  ...
  test('should return 304 Not Modified for matching ETag on model.getAll', async ({ request }) => {
    // First request to get ETag
  ...
  test('should return Cache-Control header on model.getAvailableModels', async ({ request }) => {
    const response = await request.post(`${BASE_URL}${RPC_ENDPOINT}/model/getAvailableModels`, {
      data: {},
    });
...
test.describe.skip('ORPC Caching - Chat Endpoints', () => {
  ...
  test('should return 304 Not Modified for matching ETag on chat.getAll', async ({ request }) => {
    // First request to get ETag
    ...
    if (firstResponse.status() === 200) {
      ...
      // Second request with If-None-Match header
      const secondResponse = await request.post(`${BASE_URL}${RPC_ENDPOINT}/chat/getAll`, {
        data: {},
        headers: {
          'If-None-Match': etag,
        },
...
test.describe('ORPC Caching - Cache Duration Consistency', () => {
  test('should have consistent cache durations across procedures', async () => {
    ...
    expect(expectedDurations['model.getAll']).toBeGreaterThan(expectedDurations['chat.getAll']);
...
test.describe('ORPC Caching - Security', () => {
  test('should use private cache directive for all endpoints', async () => {
    ...
    // ensuring user-specific data is not cached at the proxy level
    ...
    for (const [endpoint, header] of Object.entries(cacheHeaders)) {
      // All cache headers must start with 'private' directive
  ...
  test('should include no-transform directive', async () => {
    ...
    // This is a security measure to ensure response integrity
    ...
    for (const [endpoint, header] of Object.entries(cacheHeaders)) {
      // All cache headers must include 'no-transform' directive
