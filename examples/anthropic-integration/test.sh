...
# Anthropic Integration Test Script
# Tests the Anthropic provider integration

set -e

BASE_URL="${BASE_URL:-http://localhost:3001}"
echo "🧪 Testing Anthropic Integration at $BASE_URL"
echo ""
...
if [ "$http_code" = "200" ]; then
  ...
  if [ "$provider" = "anthropic" ]; then
    print_result 0 "Health check returns Anthropic provider"
  else
