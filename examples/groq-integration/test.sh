...
# Tests the Groq provider integration

set -e

BASE_URL="${BASE_URL:-http://localhost:3001}"
echo "⚡ Testing Groq Integration at $BASE_URL"
echo ""
...
if [ "$http_code" = "200" ]; then
  ...
  if [ "$provider" = "groq" ]; then
    print_result 0 "Health check returns Groq provider"
  else
...
if [ $TESTS_FAILED -eq 0 ]; then
  ...
  echo ""
  echo "⚡ Groq is ready to use!"
  echo "🚀 Enjoy ultra-fast inference!"
  exit 0
else
