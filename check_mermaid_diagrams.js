...
diagrams.forEach((diagram, index) => {
  ...
  if (diagram.lineCount > 100) {
    warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Very large (${diagram.lineCount} lines) - May render slowly or timeout`);
  }

  // Check 2: Check for very long lines (GitHub has width limits)
  const longLines = lines.filter(line => line.length > 200);
  if (longLines.length > 0) {
    warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Has ${longLines.length} very long lines (>200 chars) - May overflow horizontally`);
  }
  ...
  if (nestingDepth > 10) {
    warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Deep nesting (${nestingDepth} levels) - May be hard to read`);
  }

  // Check 4: Check for potential syntax issues
  if (!content.match(/^(flowchart|sequenceDiagram|classDiagram|stateDiagram|gitGraph|pie|erDiagram|gantt)/m)) {
    issues.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Missing valid diagram type declaration`);
  }
  ...
  if (openBrackets !== closeBrackets) {
    issues.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Mismatched square brackets [${openBrackets} open, ${closeBrackets} close]`);
  }
  ...
  if (openParens !== closeParens) {
    issues.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Mismatched parentheses [${openParens} open, ${closeParens} close]`);
  }
  ...
  if (subgraphStarts > subgraphEnds) {
    issues.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Unclosed subgraph [${subgraphStarts} subgraph, ${subgraphEnds} end]`);
  }
  ...
  if (controlBlocks.length > blockEnds && content.includes('sequenceDiagram')) {
    warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Possible unclosed control block [${controlBlocks.length} blocks, ${blockEnds} ends]`);
  }
  ...
  if (specialChars && specialChars.length > 50) {
    warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Many special characters (${specialChars.length}) - Ensure UTF-8 encoding`);
  }
  ...
  if (content.includes('sequenceDiagram')) {
    ...
    if (participants.length === 0) {
      warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Sequence diagram with no participants`);
    }
  }
  ...
  if (content.includes('classDiagram')) {
    ...
    if (classes.length === 0) {
      warnings.push(`Diagram #${diagram.number} (line ${diagram.startLine}): Class diagram with no classes`);
    }
  }
});
...
if (issues.length === 0 && warnings.length === 0) {
  ...
  console.log(`  • Average size: ${Math.round(diagrams.reduce((sum, d) => sum + d.lineCount, 0) / diagrams.length)} lines`);
...
diagrams.forEach(diagram => {
  const typeMatch = diagram.content.match(/^(flowchart|sequenceDiagram|classDiagram|stateDiagram|gitGraph|pie|erDiagram|gantt)/m);
  if (typeMatch) {
