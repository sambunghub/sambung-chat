...
const supportedTypes = [
  ...
  'gantt', 'pie', 'journey', 'mindmap', 'timeline', 'sankey', 'block'
];

// GitHub does NOT support these features:
const unsupportedFeatures = [
  'requirementDiagram', 'c4Context', 'c4Container', 'c4Component',
  'c4Deployment', 'c4Dynamic', 'packet beta', 'rlBlock beta',
  'architecture beta', 'netconf beta'
];
...
diagrams.forEach((diagram, index) => {
  ...
  const typeMatch = content.match(/^(flowchart|graph|sequenceDiagram|classDiagram|stateDiagram|stateDiagram-v2|erDiagram|gitGraph|gantt|pie|journey|mindmap|timeline|sankey|block)/m);
...
const typeCounts = {};
diagrams.forEach(diagram => {
  const typeMatch = diagram.content.match(/^(flowchart|graph|sequenceDiagram|classDiagram|stateDiagram|stateDiagram-v2|erDiagram|gitGraph|gantt|pie|journey|mindmap|timeline|sankey|block)/m);
  if (typeMatch) {
