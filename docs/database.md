...
export const chatUserModelUpdatedIdx = index('chat_user_model_updated_idx')
  .on(chats.userId, chats.modelId, chats.updatedAt);
...
const results = await db
  ...
  .where(
    and(
      ...
      inArray(models.id, selectedModelIds)
    )
  )
...
const searchByProvider = await db
  ...
  .where(
    and(
      eq(chats.userId, userId),
      inArray(models.provider, ['openai', 'anthropic'])
    )
  );
