...
1. **NEVER edit files in `packages/ui/src/lib/components/ui/`**
...
│   │       ├── components/  # ⚠️ TODO: Move to packages/ui
...
```svelte
<script>
  import { Button, Card, Input } from '@sambung-chat/ui';
</script>
...
```svelte
<!-- ❌ packages/ui/src/lib/components/ui/button/Button.svelte -->
...
```svelte
<!-- ✅ packages/ui/src/components/GradientButton.svelte -->
<script>
...
| Add UI component | `packages/ui/src/components/[Component].svelte` |
...
| `packages/ui/src/lib/index.ts`      | UI exports                |
...
- [ ] Check if similar component already exists in `@sambung-chat/ui`
