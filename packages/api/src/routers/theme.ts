...
const hslColorSchema = z
  .string()
  .regex(/^\d+\s+\d+%\s+\d+%$/, 'Color must be in HSL format (e.g., "210 100% 50%")');
...
export const themeRouter = {
  ...
  getTheme: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      const themeResults = await db
        .select()
        .from(theme)
        .where(and(eq(theme.id, input.id), or(eq(theme.userId, userId), isNull(theme.userId))));
  ...
  updateTheme: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      // Update theme
      const updated = await db.update(theme).set(data).where(eq(theme.id, id)).returning();
