...
>
	...
	<div class="p-4 border-b border-border">
		...
		{:else}
			<div class={cn('font-semibold', 'transition-opacity duration-200', isTablet && 'text-sm:opacity-0')}>
				Settings
			</div>
		{/if}
	</div>
	...
	{#if context === 'chat' && footer}
		<div class={cn('p-4 border-t border-border', 'transition-opacity duration-200', isTablet && 'text-xs:opacity-0')}>
			{@render footer()}
		</div>
	{/if}
</aside>
